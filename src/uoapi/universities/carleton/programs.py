...
class CarletonProgramsProvider:
    ...
    def _parse_program_data(self, program_data: Dict[str, Any]) -> Program:
        ...
        # Map degree type
        degree_type = CARLETON_DEGREE_TYPE_MAPPING.get(
            degree, ProgramDegreeType.BACHELOR
        )
        ...
        program_code = (
            title.upper().replace(" ", "").replace("(", "").replace(")", "")[:10]
        )
    ...
    def get_programs(self) -> List[Program]:
        ...
        for program_data in raw_data:
            try:
                ...
                logger.warning(
                    f"Failed to parse program {program_data.get('title', 'Unknown')}: {e}"
                )
                continue
